...
% ===================================================================
% FLOWCHART — hitam putih, landscape, semua teks dijamin tidak terpotong
% Gunakan environment sidewaysfigure agar lebar penuh
% ===================================================================
\begin{figure}[H]
\centering
\resizebox{0.92\textwidth}{!}{%
\begin{tikzpicture}[
    % ---- spacing antar node utama ----
...
% ============================================================
% KOLOM TENGAH (main flow)
% ============================================================
\node (start)   [startstop]                         {START};
\node (init)    [process,  below=0.7cm of start]    {Inisialisasi App \&\\Muat Background};
\node (input)   [process,  below=0.7cm of init]     {Baca Input Slider\\Operator};
\node (noise)   [process,  below=0.7cm of input]    {Tambah Noise (rand)\\+ Filter MA};
\node (bpcs)    [process,  below=0.7cm of noise]    {Eksekusi Logika BPCS};

\node (dec1)    [decision, below=0.9cm of bpcs]     {PV $\geq$ 90\%?};
\node (dec2)    [decision, below=1.4cm of dec1]     {PV $\leq$ 10\%?};

\node (render)  [process,  below=1.4cm of dec2]     {Render GUI di Atas\\Background P\&ID};
\node (repaint) [process,  below=0.7cm of render]   {Request Repaint};

% ============================================================
% NODE SAMPING — interlock dec1
% ============================================================
\node (cL1) [sidenode, left=3.2cm of dec1]  {SIS: Tutup\\Inlet SDV};
\node (cR1) [sidenode, right=3.2cm of dec1] {SIS: Buka\\Inlet SDV};

% NODE SAMPING — interlock dec2
\node (cL2) [sidenode, left=3.2cm of dec2]  {SIS: Tutup\\Outlet SDV};
\node (cR2) [sidenode, right=3.2cm of dec2] {SIS: Buka\\Outlet SDV};
...
\draw [arrow] (noise)   -- (bpcs);
\draw [arrow] (bpcs)    -- (dec1);
\draw [arrow] (dec1)    -- node[left, font=\scriptsize\bfseries]{Tidak} (dec2);

% ============================================================
% PERCABANGAN dec1
% ============================================================
\draw [arrow] (dec1.west)  -- node[above, font=\scriptsize\bfseries]{Ya}    (cL1.east);
\draw [arrow] (dec1.east)  -- node[above, font=\scriptsize\bfseries]{Tidak} (cR1.west);

% ============================================================
% PERCABANGAN dec2
% ============================================================
\draw [arrow] (dec2.west)  -- node[above, font=\scriptsize\bfseries]{Ya}    (cL2.east);
\draw [arrow] (dec2.east)  -- node[above, font=\scriptsize\bfseries]{Tidak} (cR2.west);

% ============================================================
% SEMUA SISI KIRI GABUNG KE render.west
% ============================================================
\coordinate (joinL) at ($(render.west) + (-3.8, 0)$);
\draw [line]  (cL1.south) -- ++(0,-0.35) -| (joinL);
\draw [line]  (cL2.south) -- ++(0,-0.35) -| (joinL);
\draw [arrow] (joinL)     -- (render.west);

% ============================================================
% SEMUA SISI KANAN GABUNG KE render.east
% ============================================================
\coordinate (joinR) at ($(render.east) + (3.8, 0)$);
\draw [line]  (cR1.south) -- ++(0,-0.35) -| (joinR);
\draw [line]  (cR2.south) -- ++(0,-0.35) -| (joinR);
\draw [arrow] (joinR)     -- (render.east);

% ============================================================
% LOOP BALIK ke input
% ============================================================
\draw [arrow] (render)  -- (repaint);
\draw [line]  (repaint.west) -- ++(-5.6, 0) coordinate (loopL);
\draw [arrow] (loopL)        |- (input.west);

\end{tikzpicture}
}% end resizebox
\caption{Flowchart Arsitektur Sistem GUI, BPCS, dan SIS}
\label{fig:flowchart}
\end{figure}
